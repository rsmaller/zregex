...
fn emitRecursive(allocator: anytype, labels: *std.ArrayList(usize), instructions: *std.ArrayList(Instruction),
    ...
    switch (ast.*) {
        ...
        .repetition => |rep| {
            ...
            _ = try emitInstruction(allocator, instructions, fixups, .{.permit_fixups = false}, .{.repeat_end = .{.repeat_start_jmp = rep_start_index}}, instruction_ptr);
                    ...
                    switch(grp_type) {
                        ...
                        .lookbehind => |look| {
                            if (grp.negated) {
                                ...
                                try emitRecursive(allocator, labels, instructions, fixups, grp.expr, instruction_ptr, recursion_level + 1);
                                try emitLabel(allocator, labels, instruction_ptr);
                                _ = try emitInstruction(allocator, instructions, fixups, .{.permit_fixups = false}, .neg_lookbehind_end, instruction_ptr);
                            } else {
                                try emitLabel(allocator, labels, instruction_ptr);
